...
| `thresholds.covered_action_confidence` | 0–1 | 0.8 | A CLICK, TYPE_TEXT or SELECT decided below this while controls sit under another layer (`covered_controls` > 0: a form still loading behind an overlay, a request in flight before a dialog opens) is **deferred once per page**: the step becomes a WAIT (`action_deferred: {operation, covered}`, flag `DEFERRED-<OP>:<n>`) ending the moment the page changes (the overlay lifting or the dialog opening counts), and Jev decides again; on the same page the next decision is executed whatever it is (a layer that stays is a dialog, and its controls are the controls). Live: the first name went into the sidebar's menu filter at 0.54–0.67 in every PIM run while the form's fields were covered (the right fields read 0.84–0.99), and a Leave flow clicked the next tab at 0.74 while the spinner before its confirmation dialog covered the form, three runs of three. `0` turns it off |
