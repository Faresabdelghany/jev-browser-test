...
| `done_unverified` | Jev said DONE confidently; the runner settled, re-observed (again while the page kept changing between looks or stayed busy, covered controls or an empty document, `recheck_again` on those steps, twice at most), and no pass outcome is visible | Either the outcome wording is off (test issue) or the app did not do what it claims (bug). Look at `final.png`. Timing is ruled out by the recheck unless the last two steps still carry `recheck_again`: then the page never held still, raise `settle_ms` |
