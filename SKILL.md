...
| `evidence.line` is null although the outcome and `evidence.present` are right | The `when` names several facts in one clause, or an absence, and no single line states all of it. Write the facts as sentences (". " between them, the most quotable first): each sentence gets its own evidence Choice in the same request and the one Jev is most confident about is quoted. An absence ("no modal covers the page") has no line to quote; `present` is its evidence |
